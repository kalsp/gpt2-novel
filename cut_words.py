...
def cut_words(processer_num, text, result_dict):
    with open(os.path.join(configs.data.path, f'raw.cut.temp.{processer_num}.txt'), 'w',encoding="utf-8") as out_f:
        texts = text.split('\n')
...
def preprocess(n_processes):
    ...
    with open(configs.data.raw, 'r',encoding="utf-8") as f:
    ...
    with open(configs.data.raw_cut, 'w',encoding="utf-8") as all_cut_file:
        for filename in path.glob('raw.cut.temp.*'):
            with open(filename,encoding="utf-8") as cut_file:
